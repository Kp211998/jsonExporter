...
st.markdown("""
This app connects to Enterprise Architect (EA), retrieves packages from your project, and generates a JSON graph (nodes & edges) for a selected package.
""")

# Create a placeholder at the top for the download button
    ...
    if valid_packages:
        ...
        if st.button("Generate JSON Graph"):
            ...
            if selected_pkg:
                ...
                st.session_state["graph_json"] = json_str  # Save JSON in session state

                st.success("JSON graph created!")
                st.json(graph)

                # Place a download button at the top if a JSON graph exists
                download_placeholder.download_button("Download JSON (Top)", data=json_str, file_name="ea_graph.json",
                                                     mime="application/json")

                # Also place a download button at the bottom
                st.download_button("Download JSON (Bottom)", data=json_str, file_name="ea_graph.json",
                                   mime="application/json")
            else:
